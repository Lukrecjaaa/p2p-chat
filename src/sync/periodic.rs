...
impl SyncEngine {
    ...
    async fn handle_dht_query_result(
        &mut self,
        key: kad::RecordKey,
        result: DhtQueryResult,
    ) -> Result<()> {
    ...
    async fn process_mailbox_messages(
        &self,
        ...
        for encrypted_msg in messages {
            ...
            if let Err(e) = self.ui_notify_tx.send(UiNotification::NewMessage(message)) {
                error!(
                    "Failed to send UI notification for new message {}: {}",
                    encrypted_msg.id, e
                );
            }
    ...
    async fn reconstruct_message_from_mailbox(
        &self,
